...
##array of all the ontologies - this is only for the all combined with the matching run_ner_all_combined
declare -a arr=('0_all_combined')
...
do
    ...
    ##move the training file to the test.tsv:
    mkdir $tokenized_files$i/$biobert_training
    cp $model_info$i$biobert$train $tokenized_files$i/$biobert_training/test.tsv

    #move the train_dev file also
    cp $model_info$i$biobert$train_dev $tokenized_files$i/$biobert_training/
...
#    NER_DIR='/scratch/Users/mabo1182/epistemic_classification/Output_Folders/Tokenized_Files/BIOBERT/'
    ...
    NER_DIR=$tokenized_files$i/$biobert_training/
    ...
    python3 biobert/run_ner_all_combined.py --do_train=true --do_predict=true --vocab_file=$biobert_original/vocab.txt --bert_config_file=$biobert_original/bert_config.json --init_checkpoint=$OUTPUT_DIR$model$global_step  --mmax_seq_length=410 --num_train_epochs=1.0 --data_dir=$NER_DIR --output_dir=$OUTPUT_DIR
