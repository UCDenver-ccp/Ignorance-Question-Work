...
#    ##loop over each ontology and run the corresponding model
    ...
    do
        ...
        echo $i
        results_path=$fiji_results_span_detection$i$biobert_training
        local_path=$local_results$i$biobert_training
        scp mabo1182@fiji.colorado.edu:$results_path/*_test.txt* $local_path

        ##training files
        scp mabo1182@fiji.colorado.edu:$fiji_tokenized_files$i/$biobert_training/* $concept_recognition_master_path$output_folders$tokenized_files$i/$biobert_training/
...
#    declare -a ont=('full_unknown')
    ...
    do
        ...
        tokenized_files='Tokenized_Files'
        results_span_detection='Results_span_detection/'
        NER_DIR=$concept_recognition_master_path$output_folders$tokenized_files/$i/$biobert_training
        OUTPUT_DIR=$concept_recognition_master_path$output_folders$results_span_detection$i$biobert_training
        ...
        if [ $gold_standard == $true ]; then
            ...
            biobert_class_report_training='_biobert_local_eval_files_classification_report_training.txt'

            cp $OUTPUT_DIR/biobert_classification_report.txt $concept_recognition_master_path$output_folders$results_span_detection$i/$i$biobert_class_report_training
